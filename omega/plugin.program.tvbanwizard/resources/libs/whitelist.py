...
def whitelist(do):
    ...
    if do == 'edit':
        ...
        while choice not in [-1, 0]:
            choice = dialog.select("{0}: Seleccione los Add-ons que desea incluir en whitelist.".format(CONFIG.ADDONTITLE), tempaddonnames)
            if choice == -1:
                break
            elif choice == 0:
                break
        ...
        try:
            os.remove(CONFIG.WHITELIST)
            logging.log_notify(CONFIG.ADDONTITLE,
                               "[COLOR {0}]Whitelist Borrado[/COLOR]".format(CONFIG.COLOR2))
        except:
            logging.log_notify(CONFIG.ADDONTITLE,
