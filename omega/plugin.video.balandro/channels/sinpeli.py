...
def do_downloadpage(url, post=None, headers=None):
    ...
    if not url.startswith(host):
        ...
        if not data:
            if not '?s=' in url:
                if config.get_setting('channels_re_charges', default=True): platformtools.dialog_notification('SinPeli', '[COLOR cyan]Re-Intentanto acceso[/COLOR]')

                timeout = config.get_setting('channels_repeat', default=30)

                if hay_proxies:
                    data = httptools.downloadpage_proxy('sinpeli', url, post=post, headers=headers, timeout=timeout).data
                else:
