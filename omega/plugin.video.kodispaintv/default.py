    ...
    try:
        ...
        if encoding is None:
            ...
            if r:
                ...
                r = re.search(epattern, result, re.IGNORECASE)
                if r:
                    encoding = r.group(1).decode(
                        'utf8') if six.PY3 else r.group(1)
        ...
        if encoding is not None:
            ...
            result = result.decode(
                'latin-1', errors='ignore') if six.PY3 else result.encode('utf-8')
        response.close()
            ...
            if len(sources) > 1:
                for i in sources:
                    try:
                        ...
                        if 'credits' in i:
                            credits = i['credits']
                        title = i['title'].encode(
                            'utf-8') if six.PY2 else i['title']
                        url = i['url'].encode('utf-8') if six.PY2 else i['url']
                        # url = url + "fix" if url.endswith(".xml") and six.PY3 else url
                        addDir(title, url, 1, thumb, fanart, desc,
                               genre, date, credits, 'source')
                    except:
                        traceback.print_exc()
            else:
                if len(sources) == 1:
                    if isinstance(sources[0], list):
                        getData(sources[0][1].encode('utf-8'),
                                FANART) if six.PY2 else sources[0][1]
                    else:
...
def addSource(url=None):
    ...
    if url is not None:
        if 'community-links' in url:
            xbmc.executebuiltin(
                "XBMC.Container.Update({0}?mode=10,replace)".format(sys.argv[0]))
    else:
        addon.openSettings()
    ...
    if url.startswith('http://') or url.startswith('https://'):
        ...
        if xbmcvfs.exists(url):
            if url.startswith("smb://") or url.startswith("nfs://"):
                copy = xbmcvfs.copy(url, os.path.join(
                    profile, 'temp', 'source_temp.txt'))
                if copy:
                    if six.PY2:
                        data = open(os.path.join(profile, 'temp',
                                    'source_temp.txt'), "r").read()
                    else:
                        data = open(os.path.join(
                            profile, 'temp', 'source_temp.txt'), "r", encoding='utf-8').read()
            ...
            for channel in channels:
                ...
                try:
                    ...
                    if linkedUrl == '':
                        ...
                        addDir(name, url, 2, thumbnail, fanArt,
                               desc, genre, date, credits, True)
                    else:
                        linkedUrl = linkedUrl.encode(
                            'utf-8') if six.PY2 else linkedUrl
                        addDir(name, linkedUrl, 1, thumbnail, fanArt,
    ...
    for other, channel_name, stream_url in match:
        ...
        if 'type' in other:
            ...
            if mode_type == 'yt-dl':
                ...
                addLink(url[0], channel_name, thumbnail, '',
                        '', '', '', '', None, regexs, total)
                continue
        elif tsdownloader and '.ts' in stream_url:
            stream_url = 'plugin://plugin.video.f4mTester/?url={0}&amp;streamtype=TSDOWNLOADER&name={1}'.format(
                urllib_parse.quote_plus(stream_url), urllib_parse.quote(channel_name))
        elif hlsretry and '.m3u8' in stream_url:
            stream_url = 'plugin://plugin.video.f4mTester/?url={0}&amp;streamtype=HLSRETRY&name={1}'.format(
                urllib_parse.quote_plus(stream_url), urllib_parse.quote(channel_name))
        addLink(stream_url, channel_name, thumbnail,
                '', '', '', '', '', None, '', total)
...
def getItems(items, fanart, dontLink=False):
    ...
    for item in items:
            ...
            if len(item.findall('link')) > 0:
                ...
                for i in item.findall('sportsdevil'):
                    if i.text is not None:
                        ...
                        sportsdevil = sd_plugin + '/?mode=1&amp;item=catcher%3dstreams%26url=' + \
                            i.text + '%26videoTitle=' + name
                        if item.find('referer'):
                            sportsdevil = sportsdevil + '%26referer=' + \
                                item.find('referer').text
                ...
                for i in item.findall('f4m'):
                    if i.text is not None:
                        if '.f4m' in i.text:
                            f4m = 'plugin://plugin.video.f4mTester/?url=' + \
                                urllib_parse.quote_plus(i.text)
                        elif '.m3u8' in i.text:
                            f4m = 'plugin://plugin.video.f4mTester/?url=' + \
                                urllib_parse.quote_plus(
                                    i.text) + '&amp;streamtype=HLS'
                        else:
                            f4m = 'plugin://plugin.video.f4mTester/?url=' + \
                                urllib_parse.quote_plus(
                                    i.text) + '&amp;streamtype=SIMPLE'
                    url.append(f4m)
        ...
        if isinstance(item.find('date'), ElementTree.Element):
            ...
            if isinstance(item.find('year'), ElementTree.Element):
                date = item.find('year').text
            else:
                date = ''

        if tmdbId is not None:
            try:
                from resources.lib.scrappers import TMDBScrapper
                mediatype = item.find('mediatype', None)
                if mediatype is not None:
                    data = [{elem.tag: processPyFunction(elem.text)
                            for elem in item.iter() if elem is not item}]
                    tmdbInfo = TMDBScrapper().set_itemlist_info(data)
                    name = tmdbInfo.get('title')
                    desc = tmdbInfo.get('plot')
                    thumbnail = tmdbInfo.get('poster_path')
                    if tmdbInfo.get('backdrop_path'):
                        fanArt = tmdbInfo.get('backdrop_path')
                    genre = tmdbInfo.get('genres')
                    date = tmdbInfo.get('year')
                    extraInfo = copy.deepcopy(tmdbInfo)
                    if not 'tmdb_id' in extraInfo:
            ...
            if len(url) > 1:
                ...
                for i in url:
                    if add_playlist == "false" and not ignorelistsetting:
                        alt += 1
                        addLink(i, '{0}) {1}'.format(alt, name.encode('utf-8', 'ignore') if six.PY2 else name),
                                thumbnail, fanArt, desc, genre, date, True, playlist, regexs, total)
                ...
                if len(playlist) > 1:
                    addLink('', name.encode('utf-8') if six.PY2 else name, thumbnail,
                            fanArt, desc, genre, date, True, playlist, regexs, total)
            else:
                if dontLink:
                ...
                if isXMLSource:
                    ...
                    if regexs is not None:  # <externallink> and <regex>
                        addDir(name, ext_url[0], 1, thumbnail, fanArt, desc,
                               genre, date, None, '!!update', regexs, url[0])
                    else:
                        addDir(name, ext_url[0], 1, thumbnail, fanArt,
                               desc, genre, date, None, 'source', None, None)
                elif isJsonrpc:
                    addDir(name.encode('utf-8') if six.PY2 else name,
                           ext_url[0], 53, thumbnail, fanArt, desc, genre, date, None, 'source')
                else:
                    try:
...
def evaluatePyFunction(evaluateTag, link, cookieJar, m):
    val = ''
    if evaluateTag.startswith('$pyFunction:'):
        val = doEval(evaluateTag.split('$pyFunction:')[1], link, cookieJar, m)
    else:
        val = doEvalFunction(evaluateTag, link, cookieJar, m)
    return val
        ...
        if k in regexs:
            ...
            if 'cookiejar' in m:  # so either create or reuse existing jar
                ...
                if '$doregex' in cookieJarParam:
                    cookieJar = getRegexParsed(
                        regexs, m['cookiejar'], cookieJar, True, True, cachedPages)
                    cookieJarParam = True
                else:
                    cookieJarParam = True
            ...
            if m['page'] and '$doregex' in m['page']:
                pg = getRegexParsed(
                    regexs, m['page'], cookieJar, recursiveCall=True, cachedPages=cachedPages)
                if len(pg) == 0:
                    pg = 'http://regexfailed'
                m['page'] = pg

            if 'setcookie' in m and m['setcookie'] and '$doregex' in m['setcookie']:
                m['setcookie'] = getRegexParsed(
                    regexs, m['setcookie'], cookieJar, recursiveCall=True, cachedPages=cachedPages)
            if 'appendcookie' in m and m['appendcookie'] and '$doregex' in m['appendcookie']:
                m['appendcookie'] = getRegexParsed(
                    regexs, m['appendcookie'], cookieJar, recursiveCall=True, cachedPages=cachedPages)

            if 'post' in m and '$doregex' in m['post']:
                m['post'] = getRegexParsed(
                    regexs, m['post'], cookieJar, recursiveCall=True, cachedPages=cachedPages)

            if 'rawpost' in m and '$doregex' in m['rawpost']:
                m['rawpost'] = getRegexParsed(
                    regexs, m['rawpost'], cookieJar, recursiveCall=True, cachedPages=cachedPages, rawPost=True)

            if 'rawpost' in m and '$epoctime$' in m['rawpost']:
                m['rawpost'] = m['rawpost'].replace(
                    '$epoctime$', getEpocTime())

            if 'rawpost' in m and '$epoctime2$' in m['rawpost']:
                m['rawpost'] = m['rawpost'].replace(
                    '$epoctime2$', getEpocTime2())
                ...
                if m['page'] and m['page'] != '' and m['page'].startswith('http'):
                    ...
                    if 'appendcookie' in m:
                        ...
                        for h in cookiestoApend:
                            n, v = h.split('=')
                            w, n = n.split(':')
                            ck = http_cookiejar.Cookie(version=0, name=n, value=v, port=None, port_specified=False, domain=w, domain_specified=False, domain_initial_dot=False,
                    ...
                    if cookieJar is not None:
                        ...
                        if 'noredirect' in m:
                            opener = urllib_request.build_opener(
                                cookie_handler, NoRedirection, urllib_request.HTTPBasicAuthHandler(), urllib_request.HTTPHandler())
                            opener = urllib_request.install_opener(opener)
                    elif 'noredirect' in m:
                        opener = urllib_request.build_opener(
                            NoRedirection, urllib_request.HTTPBasicAuthHandler(), urllib_request.HTTPHandler())
                        opener = urllib_request.install_opener(opener)
                    ...
                    try:
                        ...
                        if encoding is None:
                            epattern = r'<meta\s+http-equiv="Content-Type"\s+content="(?:.+?);\s+charset=(.+?)"'
                            epattern = epattern.encode(
                                'utf8') if six.PY3 else epattern
                        ...
                        if encoding is not None:
                            link = link.decode(
                                encoding.lower(), errors='ignore')
                            link = link.encode('utf8') if six.PY2 else link
                        else:
                            link = link.decode(
                                'latin-1', errors='ignore') if six.PY3 else link.encode('utf-8')
                    ...
                    if m['page'].startswith('$pyFunction:'):
                        val = doEval(m['page'].split('$pyFunction:')[
                                     1], '', cookieJar, m)
                        if forCookieJarOnly:
                            return cookieJar  # do nothing
                        link = val
            ...
            if '$doregex' in m['expres']:
                m['expres'] = getRegexParsed(
                    regexs, m['expres'], cookieJar, recursiveCall=True, cachedPages=cachedPages)
            if m['expres'] != '':
                if '$LiveStreamCaptcha' in m['expres']:
                    ...
                    val = evaluatePyFunction(m['expres'], link, cookieJar, m)
                    ...
                    if 'listrepeat' in m:
                        ...
                        if 'nextpage' in m:
                            if m['nextpage'].startswith('$pyFunction:') or '#$pyFunction' in m['nextpage']:
                                nextPageUrl = evaluatePyFunction(
                                    m['nextpage'], link, cookieJar, m)
                            else:
                                nextPageUrl = re.findall(m['nextpage'], link)
                                if isinstance(nextPageUrl, list):
                                    nextPageUrl = nextPageUrl[0]
                        if 'pagination' in m:
                            pagination = m['pagination']
                        return listrepeat, eval(val), m, regexs, cookieJar, nextPageUrl, pagination

                    try:
                        url = url.replace(u"$doregex[" + k + "]", val)
                    except:
                        url = url.replace(
                            "$doregex[" + k + "]", val.decode("utf-8"))
                else:
                    if 'listrepeat' in m:
                        listrepeat = m['listrepeat']
                        ...
                        if 'pagination' in m:
                            pagination = m['pagination']
                        return listrepeat, ret, m, regexs, cookieJar, nextPageUrl, pagination
...
def setKodiProxy(proxysettings=None):
    ...
    if proxysettings is None:
        xbmc.executeJSONRPC(
            '{"jsonrpc":"2.0", "method":"Settings.SetSettingValue", "params":{"setting":"network.usehttpproxy", "value":false}, "id":1}')
    else:
        ...
        if len(ps) > 3 and '@' in ps[3]:  # jairox ###proxysettings
            ...
            # jairox ###proxysettings.split('@')[-1]
            proxyPassword = ps[3].split('@')[1]

        xbmc.executeJSONRPC(
            '{"jsonrpc":"2.0", "method":"Settings.SetSettingValue", "params":{"setting":"network.usehttpproxy", "value":true}, "id":1}')
        xbmc.executeJSONRPC(
            '{"jsonrpc":"2.0", "method":"Settings.SetSettingValue", "params":{"setting":"network.httpproxytype", "value":' + str(proxyType) + '}, "id":1}')
        xbmc.executeJSONRPC(
            '{"jsonrpc":"2.0", "method":"Settings.SetSettingValue", "params":{"setting":"network.httpproxyserver", "value":"' + str(proxyURL) + '"}, "id":1}')
        xbmc.executeJSONRPC(
            '{"jsonrpc":"2.0", "method":"Settings.SetSettingValue", "params":{"setting":"network.httpproxyport", "value":' + str(proxyPort) + '}, "id":1}')

        if proxyUsername is not None:
            xbmc.executeJSONRPC(
                '{"jsonrpc":"2.0", "method":"Settings.SetSettingValue", "params":{"setting":"network.httpproxyusername", "value":"' + str(proxyUsername) + '"}, "id":1}')
            xbmc.executeJSONRPC(
                '{"jsonrpc":"2.0", "method":"Settings.SetSettingValue", "params":{"setting":"network.httpproxypassword", "value":"' + str(proxyPassword) + '"}, "id":1}')
...
def getConfiguredProxy():
    ...
    if proxyActive:  # PROXY_HTTP
        ...
        proxyPort = six.text_type(kodiJsonRequest({'jsonrpc': '2.0', "method": "Settings.GetSettingValue", "params": {
...
def get_unpacked(page_value, regex_for_text='', iterations=1, total_iteration=1):
    try:
        ...
        if regex_for_text and len(regex_for_text) > 0:
            try:
                page_value = re.compile(regex_for_text).findall(
                    page_value)[0]  # get the js variable
            except:
                return 'NOTPACKED'
...
def doEvalFunction(fun_call, page_data, Cookie_Jar, m):
    try:
        ...
        LSProdynamicCode = import_by_string(
            filename.split('.')[0], filenamewithpath)
...
# 1 for get, 2 for post, 3 for rawpost
def getGoogleRecaptchaResponse(captchakey, cj, type=1):
    recapChallenge = ""
    solution = ""
    ...
    if len(captchakey) > 0:  # new shiny captcha!
        ...
        if not captcha_url.startswith('http'):
            captcha_url = 'https://www.google.com/recaptcha/api/challenge?k=' + \
                captcha_url + '&ajax=1'
        ...
        captcha_reload += recapChallenge + '&k=' + \
            captcha_k + '&reason=i&type=image&lang=en'
        captcha_reload_js = getUrl(captcha_reload, cookieJar=cj)
        captcha_reload_response_chall = re.findall(
            cap_image_reg, captcha_reload_js)[0]
        captcha_image_url = 'https://www.google.com/recaptcha/api/image?c=' + \
            captcha_reload_response_chall
...
def askCaptcha(m, html_page, cookieJar):
    ...
    expre = m['expres']
    page_url = m['page']
    captcha_regex = re.compile(
        r'\$LiveStreamCaptcha\[([^\]]*)\]').findall(expre)[0]
    ...
    if '$$LSPlayOnlyOne$$' in mu_playlist[0]:
        ...
        for i in mu_playlist:
            if '$$lsname=' in i:
                ...
                names.append(d_name)
                mu_playlist[iloop] = i.split('$$lsname=')[
                    0] + ('&regexs' + i.split('&regexs')[1] if '&regexs' in i else '')
            else:
            ...
            if progress.iscanceled():
                return
            progress.update(iloop / len(mu_playlist) * 100,
                            "", "Link#%d" % (iloop), playname)
            ...
            if "&mode=19" in mu_playlist[index]:
                ...
                liz.setInfo(type='Video', infoLabels={
                            'Title': playname, 'mediatype': 'video'})
                liz.setProperty("IsPlayable", "true")
                urltoplay = urlsolver(mu_playlist[index].replace(
                    '&mode=19', '').replace(';', ''))
...
def addDir(name, url, mode, iconimage, fanart, description, genre, date, credits, showcontext=False, regexs=None, reg_url=None, allinfo={}):
    ...
    if regexs and len(regexs) > 0:
        u = sys.argv[0] + "?url=" + urllib_parse.quote_plus(url) + "&mode=" + str(mode) + "&name=" + urllib_parse.quote_plus(
            name) + "&fanart=" + urllib_parse.quote_plus(fanart) + "&regexs=" + regexs
    else:
        u = sys.argv[0] + "?url=" + urllib_parse.quote_plus(url) + "&mode=" + str(
            mode) + "&name=" + urllib_parse.quote_plus(name) + "&fanart=" + urllib_parse.quote_plus(fanart)
    ...
    if len(allinfo) < 1:
        liz.setInfo(type="Video", infoLabels={"Title": name, 'mediatype': 'video',
                    "Plot": description, "Genre": genre, "dateadded": date, "credits": credits})
    else:
    ...
    if showcontext:     # Traslate by @joheos11
        ...
        if showcontext == 'source':
            ...
            contextMenu.append(('Eliminar de Favoritos KodispainTv', 'RunPlugin(%s?mode=6&name=%s)'
                                % (sys.argv[0], urllib_parse.quote_plus(name))))
        ...
        if name not in FAV:
            contextMenu.append(('Añadir a Favoritos KodispainTv', 'RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def pluginquerybyJSON(url, give_me_result=None, playlist=False):
    if 'audio' in url:
        json_query = uni(
            '{"jsonrpc":"2.0","method":"Files.GetDirectory","params": {"directory":"%s","media":"video", "properties": ["title", "album", "artist", "duration","thumbnail", "year"]}, "id": 1}') % url
    else:
        json_query = uni(
            '{"jsonrpc":"2.0","method":"Files.GetDirectory","params":{"directory":"%s","media":"video","properties":[ "plot","playcount","director", "genre","votes","duration","trailer","premiered","thumbnail","title","year","dateadded","fanart","rating","season","episode","studio","mpaa"]},"id":1}') % url
    ...
    if 'error' in json_folder_detail:
        ...
        for i in json_folder_detail['result']['files']:
            ...
            meta = dict((k, v) for k, v in six.iteritems(
                i) if not v == '0' or not v == -1 or v == '')
            meta.pop("file", None)
            if i['filetype'] == 'file':
                if playlist:
                    ...
                    addLink(url, name, thumbnail, fanart, '', '', '', '', None, '', total=len(
                        json_folder_detail['result']['files']), allinfo=meta)
...
def addLink(url, name, iconimage, fanart, description, genre, date, showcontext, playlist, regexs, total, setCookie="", allinfo={}, **kwargs):
    contextMenu = []
    parentalblock = addon.getSetting('parentalblocked')
    parentalblock = parentalblock == "true"
    parentalblockedpin = addon.getSetting('parentalblockedpin')

    if kwargs.get('trailer'):
        kwargs['trailer'] = json.dumps(kwargs.get('trailer'))
        contextMenu.append(('Ver Trailer', 'RunPlugin(%s?mode=54&url=%s)' % (
    ...
    if 'plugin://plugin.video.youtube/play/?video_id=' in url:
        yt_audio_url = url.replace(
            'plugin://plugin.video.youtube/play/?video_id=', 'https://www.youtube.com/watch?v=')
    ...
    if playlist:
        ...
        u += "url=" + \
            urllib_parse.quote_plus(url) + "&name=" + \
            urllib_parse.quote(name) + "&mode=" + mode
    else:
        u += "url=" + urllib_parse.quote_plus(url) + "&mode=" + mode
    if regexs:
        u += "&regexs=" + regexs
    ...
    if iconimage and iconimage != '':
        ...
        u += "&iconimage=ICON"
    if kwargs.get('tmdb_id', None) is not None:
        u += "&tmdb=%s" % (str(kwargs.get('tmdb_id')))
    ...
    if date == '':
        ...
        description += '\n\nDate: %s' % date
    liz = xbmcgui.ListItem(name)
    liz.setArt({'thumb': iconimage if iconimage and iconimage != "ICON" else icon,
                'fanart': fanart,
                'icon': "DefaultVideo.png"})
    if kwargs:
        if 'genres' in kwargs:
        ...
        if 'cast' in kwargs:
            kwargs['cast'] = [(cast['name'], cast['character'])
                              for cast in kwargs['cast']]
        liz.setInfo(type="Video", infoLabels=kwargs)
    elif allinfo is None or len(allinfo) < 1:
        liz.setInfo(type="Video", infoLabels={
                    "Title": name, 'mediatype': 'video', "Plot": description, "Genre": genre, "dateadded": date})
    else:
        allinfo.update({'mediatype': 'video'})
        liz.setInfo(type="Video", infoLabels=allinfo)
    ...
    if showcontext: #Traslate by @joheos11
        if showcontext == 'fav':
            contextMenu.append(
                ('Eliminar de Favoritos KodispainTv', 'RunPlugin(%s?mode=6&name=%s)'    # Traslate by @joheos11
                 % (sys.argv[0], urllib_parse.quote_plus(name)))
            )
            ...
            try:
                ...
                fav_params = (
                    ...
                    % (sys.argv[0], urllib_parse.quote_plus(name), urllib_parse.quote_plus(url),
                       urllib_parse.quote_plus(iconimage.encode(
                           "utf-8") if six.PY2 else iconimage),
                       urllib_parse.quote_plus(fanart.encode("utf-8") if six.PY2 else fanart))
                )
            if playlist:
                fav_params += 'playlist=' + \
                    urllib_parse.quote_plus(str(playlist).replace(',', '||'))
            if regexs:
                fav_params += "&regexs=" + regexs
            contextMenu.append(
                ('Añadir a Favoritos KodispainTv', 'RunPlugin(%s)' % fav_params))
        liz.addContextMenuItems(contextMenu)
    try:
    ...
    ok = xbmcplugin.addDirectoryItem(handle=int(
        sys.argv[1]), url=u, listitem=liz, totalItems=total, isFolder=isFolder)
    return ok
...
def playsetresolved(url, name, iconimage, setresolved=True, reg=None):
    ...
    if '&mode=22' in url:
        ...
        u += "url=" + \
            urllib_parse.quote_plus(url) + "&name=" + \
            urllib_parse.quote(name) + "&mode=22"
        url = u
...
def play_duff(ytId):
    # Thanks to "La Taberna"
    import duffyou
    duffyou.play(ytId, title="", plot="", thumb="")
...
def d2x(d, root="root", nested=0):
    def op(tag): return '<' + tag + '>'  # noQA
    def cl(tag): return '</' + tag + '>\n'  # noQA
    def ml(v, xml): return xml + op(key) + str(v) + cl(key)  # noQA
    xml = op(root) + '\n' if root else ""
...
try:
    ...
    pass
try:
    playlist = eval(urllib_parse.unquote_plus(
        params["playlist"]).replace('||', ','))
except:
    pass
try:
...
try:
    ...
    pass
try:
    tmdbId = urllib_parse.unquote_plus(params["tmdb"])
except:
    tmdbId = None
...
if mode is None:
    ...
    if not url.startswith("plugin://plugin") or not any(x in url for x in g_ignoreSetResolved):
        ...
        if '$PLAYERPROXY$=' in url:
            ...
            playmediawithproxy(url, name, iconimage, proxyip,
                               port, proxyuser, proxypass)  # jairox
    ...
    if regexs and 'listrepeat' in urllib_parse.unquote_plus(regexs):
        listrepeat, ret, m, regexs, cookieJar, nextPageUrl, pagination = getRegexParsed(
            regexs, url)
        d = ''
        regexname = m['name']
        url = ''
        ln = ''
        rnumber = 0
        if nextPageUrl is not None or pagination is not None:
            regexcopy = copy.deepcopy(regexs)
        existing_list = regexs.pop(regexname)
        if pagination is not None:
            pagination = int(pagination)
            original_ret = copy.deepcopy(ret)
            ret = ret[pagination * perpage:perpage + pagination * perpage]
        for obj in ret:
            try:
                rnumber += 1
                ...
                if tmdbId is not None:
                    listrepeatT += '<tmdb>%s</tmdb>' % (str(tmdbId))
                i = 0
                for i in range(len(obj)):
                                ...
                                for the_key, the_value in six.iteritems(the_valueO):
                                    if the_value is not None:
                                        if type(the_value) is dict:
                                            for the_keyl, the_valuel in six.iteritems(the_value):
                                                if the_valuel is not None:
                                                    ...
                                                    if '[' + regexname + '.param' + str(i + 1) + '][DE]' in the_valuel:
                                                        the_valuel = the_valuel.replace(
                                                            '[' + regexname + '.param' + str(i + 1) + '][DE]', urllib_parse.unquote(val))
                                                    the_value[the_keyl] = the_valuel.replace(
                                                        '[' + regexname + '.param' + str(i + 1) + ']', val)
                                            ...
                                            the_valueO[the_key] = the_value.replace(
                                                '[' + regexname + '.param' + str(i + 1) + ']', val)
                    ...
                    if '[' + regexname + '.param' + str(i + 1) + '][DE]' in listrepeatT:
                        listrepeatT = listrepeatT.replace(
                            '[' + regexname + '.param' + str(i + 1) + '][DE]', val)
                    listrepeatT = listrepeatT.replace(
                        '[' + regexname + '.param' + str(i + 1) + ']', escape(val))

                listrepeatT = listrepeatT.replace(
                    '[' + regexname + '.param' + str(0) + ']', str(rnumber))
                try:
                    if cookieJar and '[' + regexname + '.cookies]' in listrepeatT:
                        listrepeatT = listrepeatT.replace(
                            '[' + regexname + '.cookies]', getCookiesString(cookieJar))
                except:
                    pass
        ...
        if regexcopy is not None and isinstance(regexcopy, dict) and len(regexcopy) > 0:
            ...
            if nextPageUrl:
                ...
                linkNpElem.text = "$doregex[{}]".format(
                    nextPageRegex.get('name'))
                titleNpElem.text = "[I]Next Page[/I]"
                itemNextPage.append(titleNpElem)
                itemNextPage.append(linkNpElem)
                regexNpElem = ElementTree.Element("regex")
        ...
        if not (regexs and 'notplayable' in regexs and not url):
            if url:
                if '$PLAYERPROXY$=' in url:
                    ...
                    playmediawithproxy(
                        url, name, iconimage, proxyip, port, proxyuser, proxypass)  # jairox
                else:
                    playsetresolved(url, name, iconimage, setresolved, regexs)
            else:
                xbmcgui.Dialog().notification(
                    addon_name, 'Failed to extract regex.', icon, 4000, False)
    ...
    try:
        ...
        if re.search(r'\$\$RESOLVEONLY\$\$', url):
            ...
            slProxy = streamlink_proxy.SLProxy_Helper()
            if m3u8mod:
                url = urllib_parse.quote(
                    url) + '&amp;m3u8mod=%s' % m3u8mod + prxy
            else:
    ...
    trailers = json.loads(base64.b64decode(url).decode('utf-8'))
    dialogSelect = xbmcgui.Dialog().select(
        addon_name, [trailer['name'] for trailer in trailers])
    selected = trailers[dialogSelect]
    play_duff(selected['url'])
